...
class BMIClac:                                                          # 外部可调用主类
    ...
    def get_weight():                                                       # 读取键盘输入体重的方法
        user_weight = input(f"{p}请输入您的体重（单位：kg）: ")                     # 读取输入后传给变量 weight
        return float(user_weight)                                               # 取浮点数后返回变量 weight

    @staticmethod
    def get_height():                                                       # 读取键盘输入身高的方法
        user_height = input(f"{p}请输入您的身高（单位：m）: ")                      # 将读取输入传给变量 height
    ...
    def main(self):                                                         # 主运行方法
        try:                                                                    # 逻辑代码
            ...
            bmi_result = format(bmi, '.2f')                                         # 定义常用格式，此处为取后 BMI 两位小数点
        except ValueError:                                                      # 检测数值错误
            print(f"{e}您输入了不合规范的字符（输入为空、输入非数字字符），请重新运行程序，并输入正确的数字。")
        except ZeroDivisionError:                                               # 检测除数为零错误
            print(f"{e}身高不能为零，请重新运行程序，并输入正确的数字。")
        except:                                                                 # 其他错误
            print(f"{e}发生未知错误，请重新运行程序。")
        else:                                                                   # try/except 检测无错误后进入 else 判断
            if bmi == 0:                                                            # 检测体重为零错误，并判断 BMI 量级
                print(f"{e}体重不能为零，请重新运行程序，并输入正确的数字。")
            elif 0.01 <= bmi < 18.5:
                print(f"{p}您的BMI值为：{bmi_result}（取后两位小数），低于正常体重。")
            elif 18.5 <= bmi < 25:
                print(f"{p}您的BMI值为：{bmi_result}（取后两位小数），属于正常体重。")
            elif 25 <= bmi < 30:
                print(f"{p}您的BMI值为：{bmi_result}（取后两位小数），属于超重。")
            elif 30 <= bmi < 35:
                print(f"{p}您的BMI值为：{bmi_result}（取后两位小数），属于一类肥胖。")
            elif 35 <= bmi < 40:
                print(f"{p}您的BMI值为：{bmi_result}（取后两位小数），属于二类肥胖。")
            elif bmi >= 40:
                print(f"{p}您的BMI值为：{bmi_result}（取后两位小数），属于三类肥胖。")
        finally:                                                                # 程序结束输出
            print(f"{p}程序运行结束。")
